...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const officeIds = Array.isArray(body.office_ids)
      ? [...new Set(body.office_ids.map(Number).filter(Boolean))]
      : [];
    const mode = String(
      body.analysis_mode ?? body.search_mode ?? '',
    ) === 'period'
      ? 'period'
      : 'invalid';
    ...
    if (officeIds.length !== 1) {
      return new Response(
        JSON.stringify({
          function_version: FUNCTION_VERSION,
          error: 'El worker requiere exactamente un office_id.',
    ...
    if (mode !== 'period') {
      return new Response(
        JSON.stringify({
          function_version: FUNCTION_VERSION,
          error: 'El worker solo acepta analysis_mode=period.',
        }),
        {
          status: 400,
          headers: {
    ...
    if (mode === 'period') {
      ...
      if (chunkDays > 30) {
        return new Response(
          JSON.stringify({
            function_version: FUNCTION_VERSION,
            error: 'El chunk no puede superar 30 días inclusivos.',
